...
const createUserController = async(req,res)=>{
    try {
        ...
        if(existingUser) throw new Error("User with same username Or Email Exists");
...
const logoutController = async(req,res)=>{
    try {
        res.clearCookie("token",{sameSite:true,secure:true}).status(200).json("User logged out successfully");
    } catch (err) {
        res.status(500).json("Internal Server Error");
    }
}
...
module.exports = {createUserController,loginController,logoutController,refetchUserController};
